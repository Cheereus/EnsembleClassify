...
from Clustering import get_GMM
...
# TODO 修改为多线程并行
@time_indicator
def rel_mat_GMM(dataset, dr_methods, n_clusters):
    for method in dr_methods:
        ...
        # 计算并输出评价指标
        print('-------------')
        print(dataset, method, 'GMM')
        print('ARI:', ARI(labels_true, labels_pred))
        print('NMI:', NMI(labels_true, labels_pred))

        # 保存聚类结果，用于绘图和其他分析
        joblib.dump(labels_pred, 'labels_pred/' + dataset + '/' + method + 'GMM.pkl')

        # 生成相关矩阵并保存，用于后续处理
        rel_mat = RelevanceMatrix(labels_pred)
        joblib.dump(rel_mat, 'rel_mat/' + dataset + '/' + method + 'GMM.pkl')


if __name__ == '__main__':
    dataset_name = 'PBMC'
    n = 6
    rel_mat_GMM(dataset_name, dimension_reduction_methods, 6)
